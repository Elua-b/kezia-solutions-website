...
export default async function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#171717]">
      ...
      {/* ── HERO ─────────────────────────────────────────────────── */}
      <section className="relative pt-16 sm:pt-20 lg:pt-28 pb-20 sm:pb-24 lg:pb-28 px-6 lg:px-10">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/contact"
              className="text-[13px] font-bold uppercase tracking-wide bg-[#F5B700] text-[#141414] px-8 py-4 rounded-md hover:bg-white transition-colors duration-300"
            >
              Get In Touch
            </Link>
            <Link
      ...
      {/* Floating feature strip — sits outside the hero's overflow-hidden so it can never be clipped, however tall its content wraps at any width */}
      <div className="relative z-10 max-w-6xl mx-auto px-6 lg:px-10 -mt-14 sm:-mt-16 lg:-mt-20">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 bg-white rounded-2xl shadow-2xl shadow-black/20 p-6 sm:p-8">
          {heroFeatures.map(({ icon: Icon, title, body }) => (
            <div key={title} className="flex flex-col sm:flex-row gap-4 px-2">
              <div className="w-12 h-12 rounded-lg bg-[#F2F2F0] flex items-center justify-center flex-shrink-0">
                <Icon className="w-5 h-5 text-[#141414]" />
              </div>
              <div>
                <h3 className="text-[14px] font-bold text-[#141414] leading-snug mb-1">{title}</h3>
                <p className="text-[12.5px] text-[#5C5C5C] leading-relaxed">{body}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <section className="px-6 lg:px-10 py-16 sm:py-20">
          <div className="max-w-7xl mx-auto">
            <div className="text-center max-w-2xl mx-auto mb-10">
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-[#141414] leading-tight">
                Flyers & Announcements
              </h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {fliers.map(({ id, title, image_url }) => (
                <div key={id} className="rounded-xl overflow-hidden shadow-sm border border-[#141414]/8 bg-white">
                  ...
                  {title && (
                    <p className="px-3 py-2.5 text-[12px] font-semibold text-[#141414] text-center truncate">
                      {title}
                    </p>
                  )}
                </div>
      ...
      <section className="px-6 lg:px-10 py-16 sm:py-20 lg:py-24">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          <div>
            <span className="text-[11px] tracking-[0.3em] uppercase text-[#F5B700]/90 bg-[#141414] inline-block px-3 py-1.5 rounded mb-5">
              Who We Are
            </span>
            <h2 className="text-3xl sm:text-4xl font-black leading-tight text-[#141414]">
              We Believe Africa's Growth Is Accelerated Through Strong Partnerships.
            </h2>
          </div>
          <div className="lg:pt-2">
            <div className="w-10 h-[3px] bg-[#F5B700] mb-6" />
            <p className="text-[15px] text-[#5C5C5C] leading-relaxed mb-4">
            ...
            <Link
              href="/about"
              className="inline-flex items-center gap-3 text-[13px] font-bold uppercase tracking-wide text-[#141414] group"
            >
              More About Us
      ...
      {/* ── TOP DIVISIONS ────────────────────────────────────────── */}
      <section className="bg-[#F2F2F0] px-6 lg:px-10 py-16 sm:py-20 lg:py-24">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {topDivisions.map(({ id, icon: Icon, title, tag, body }) => (
              <Link
                key={id}
                href={`/divisions#${id}`}
                className="group bg-white rounded-xl p-7 border border-[#141414]/6 hover:border-[#F5B700] hover:shadow-xl transition-all duration-300"
              >
                <div className="w-12 h-12 rounded-lg bg-[#141414] flex items-center justify-center mb-6 group-hover:bg-[#F5B700] transition-colors duration-300">
                  <Icon className="w-5 h-5 text-white group-hover:text-[#141414] transition-colors duration-300" />
                </div>
                <span className="text-[10px] tracking-[0.2em] uppercase text-[#F5B700] font-bold mb-2 block">
                  {tag}
                </span>
                <h3 className="text-[16px] font-bold text-[#141414] mb-2">{title}</h3>
      ...
      <section className="bg-white">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-px bg-[#141414]/8 border-y border-[#141414]/8">
          {stats.map(({ value, label }) => (
            <div key={label} className="py-10 md:py-14 text-center bg-white">
              <div className="text-3xl sm:text-4xl md:text-5xl font-black text-[#141414] mb-2">{value}</div>
              <div className="text-[10px] sm:text-[11px] tracking-[0.2em] uppercase text-[#5C5C5C]">{label}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
            ...
            <Link
              ...
              className="text-[13px] font-bold uppercase tracking-wide text-[#141414] border-b-2 border-[#F5B700] pb-1"
            >
              Learn More
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {platforms.map(({ name, place }) => (
              <div
                key={name}
                className="rounded-xl border border-[#141414]/8 p-6 hover:border-[#F5B700] hover:shadow-lg transition-all duration-300"
              >
                <Globe2 className="w-6 h-6 text-[#F5B700] mb-4" />
                <h3 className="text-[14px] font-bold text-[#141414] mb-1 leading-snug">{name}</h3>
                <p className="text-[12px] text-[#5C5C5C]">{place}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── CTA ──────────────────────────────────────────────────── */}
      <section className="relative bg-[#0A0A0A] px-6 py-20 sm:py-24 text-center overflow-hidden">
        <div
        ...
        <div className="relative max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/contact"
              className="text-[13px] font-bold uppercase tracking-wide bg-[#F5B700] text-[#141414] px-9 py-4 rounded-md hover:bg-white transition-colors duration-300"
            >
              Contact Us
            </Link>
            <Link
